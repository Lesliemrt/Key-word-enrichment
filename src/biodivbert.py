"""
BiodivBERT Module - Complete implementation for species extraction
Includes training with checkpoint saving and best epoch selection
"""
import os
import torch
import numpy as np
...
from src.utils.config import biodivbert_model_path, biodivbert_checkpoint_dir, EXTENDED_CSV_PATH, RANDOM_STATE
...
def train_biodivbert(train_df, test_df, epochs=EPOCHS, checkpoint_dir=None):
    """
    Train BiodivBERT model with checkpoint saving and best epoch selection
    ...
    """
    print(f"---- Training BiodivBERT for {epochs} epochs ----")
    
    
    ckpt_dir = checkpoint_dir if checkpoint_dir else biodivbert_checkpoint_dir
    
    tokenizer = AutoTokenizer.from_pretrained(biodivbert_model_path)
    ...
    model = AutoModelForTokenClassification.from_pretrained(biodivbert_model_path, num_labels=3)
    ...
    for ep in range(1, epochs + 1):
        total_loss = 0
        for batch in tqdm(loader, desc=f"BiodivBERT Epoch {ep}/{epochs}", leave=False):
        ...
        print(f"BiodivBERT Epoch {ep}/{epochs} | Loss: {avg_loss:.4f}")
...
def load_model():
    """Load BiodivBERT model from checkpoint or base model"""
    global _model, _tokenizer, _device
    if _model is None:
        
        if os.path.exists(biodivbert_checkpoint_dir):
            epochs = [int(d.split('_')[1]) for d in os.listdir(biodivbert_checkpoint_dir) 
                     if d.startswith('epoch_') and os.path.isdir(os.path.join(biodivbert_checkpoint_dir, d))]
            if epochs:
                best_epoch = max(epochs)
                checkpoint_path = os.path.join(biodivbert_checkpoint_dir, f"epoch_{best_epoch}")
                print(f"Loading BiodivBERT from checkpoint: {checkpoint_path}")
                ...
                print("No checkpoints found, loading base BiodivBERT...")
